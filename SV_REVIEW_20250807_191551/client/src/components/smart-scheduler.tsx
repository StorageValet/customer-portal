...
import { format, addDays, isWeekend, isSameDay, parseISO, startOfDay, isAfter, isBefore } from "date-fns";
...
export default function SmartScheduler({
  ...
  const generateTimeSlots = (date: string): TimeSlot[] => {
    ...
    const dayMovements = existingMovements.filter((movement) => {
      try {
        const movementDate = movement.scheduledDate instanceof Date 
          ? movement.scheduledDate 
          : new Date(movement.scheduledDate);
        return isSameDay(movementDate, selectedDate);
      } catch {
        return false;
      }
    });
  ...
  const getSmartDateSuggestions = () => {
    ...
    // Next available business day (minimum 1 day in future for logistics)
    let nextBusinessDay = addDays(today, 1);
    while (isWeekend(nextBusinessDay)) {
      nextBusinessDay = addDays(nextBusinessDay, 1);
    }

    // Only show "Tomorrow" if it's actually tomorrow
    const tomorrow = addDays(today, 1);
    const label = isSameDay(nextBusinessDay, tomorrow) ? "Tomorrow" : format(nextBusinessDay, "EEEE");
    
    suggestions.push({
      date: format(nextBusinessDay, "yyyy-MM-dd"),
      label: label,
      badge: "Recommended",
      color: "bg-turquoise/10 text-turquoise border-turquoise",
    });
  ...
  const handleSubmit = () => {
    ...
    if (isBefore(selectedDateObj, today)) {
      alert("Please select a future date for your " + type);
      return;
    }
    ...
    onSchedule({
      ...
      itemIds: selectedItems.map(id => id.toString()), // Ensure IDs are strings
      specialInstructions,
      urgentService,
      combineWithDelivery,
          ...
          <div className="flex items-center justify-between">
            ...
            {selectedItems.length > 0 && (
              <div className="text-right space-y-1">
                ...
                <div className="text-sm text-gray-regent">Est. {estimations.time} minutes</div>
                <div className="text-sm space-x-3">
                  <span className="text-charcoal">{estimations.itemCount} items</span>
                  <span className="text-charcoal">{estimations.totalVolume} ft³</span>
                  <span className="text-charcoal">{estimations.totalWeight} lbs</span>
                </div>
          ...
          {availableItems.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-regent">
                ...
                {type === "pickup" ? "pickup at home" : "delivery from storage"}.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              {availableItems.map((item) => (
                <div
                  key={item.id}
                  className={`p-3 md:p-4 border rounded-lg cursor-pointer transition-all transform hover:scale-[1.02] ${
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-2 text-xs text-charcoal">
                        <span>
                          {item.length}x{item.width}x{item.height}" • {item.weight} lbs
                        </span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-2 md:gap-3">
              {datesuggestions.map((suggestion) => (
                <Button
                  ...
                  className={`h-auto p-3 md:p-4 flex flex-col items-start btn-hover-lift transition-all ${
                    selectedDate === suggestion.date
                      ? "bg-teal text-white hover:bg-teal-medium border-teal"
                      : "text-navy border-gray-200 hover:border-teal bg-white"
                  }`}
                ...
                >
                  <div className="w-full flex items-center justify-between mb-1">
                    ...
                    {suggestion.badge && (
                      <Badge className={suggestion.color}>{suggestion.badge}</Badge>
                    )}
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              {timeSlots.map((slot) => (
                <Button
                  ...
                  className={`h-auto p-3 md:p-4 flex flex-col md:flex-row items-start md:items-center justify-between btn-hover-lift transition-all ${
                ...
                >
                  <div className="flex items-center space-x-2 md:space-x-3">
                    ...
                    <div className="text-left">
                      <div className="font-medium text-sm md:text-base">{slot.label}</div>
                      {!slot.available && <div className="text-xs md:text-sm text-red-500">Unavailable</div>}
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-1 md:gap-2 mt-2 md:mt-0">
                    {slot.recommended && (
                      <Badge className="bg-emerald/10 text-emerald border-emerald text-xs">
                        <Star className="mr-1 h-3 w-3" />
                        Best
                      </Badge>
                    )}
                    {slot.premium && (
                      <Badge className="bg-purple/10 text-purple border-purple text-xs">
          ...
          <CardContent className="p-6 space-y-4">
            <div className="flex items-center space-x-3">
              <Checkbox
                ...
                onCheckedChange={(checked) => setUrgentService(checked === true)}
              />
              <Label htmlFor="urgent" className="text-navy cursor-pointer">
            ...
            {type === "pickup" && (
              <div className="flex items-center space-x-3">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setCombineWithDelivery(checked === true)}
                />
                <Label htmlFor="combine" className="text-navy cursor-pointer">
            ...
            <div className="space-y-2">
              <Label htmlFor="instructions" className="text-navy">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-regent">
                  {selectedItems.length} items • {format(parseISO(selectedDate), "MMM d, yyyy")} •{" "}
                  {selectedTimeSlot}
                </p>
              </div>
              <div className="text-right">
                ...
                <div className="text-sm text-gray-regent">Estimated {estimations.time} minutes</div>
              </div>
            </div>
            ...
            <Alert className="mb-4">
              ...
              <AlertDescription>
                You'll receive email confirmation and SMS updates about your {type} appointment. Our
                team will arrive within your selected time window.
              </AlertDescription>
            </Alert>
            ...
            <Button
              ...
              disabled={isLoading}
              className="w-full bg-teal text-navy hover:bg-teal-medium btn-hover-lift"
              size="lg"
            >
