...
export class AirtableStorage implements IStorage {
  ...
  private convertAirtableRecord(
    ...
    switch (type) {
      ...
      case "item":
        ...
        // Parse dimensions from Description field if present
        let dimensions = { length: 12, width: 12, height: 12, weight: 10 }; // defaults
        let description = fields["Description"] || null;
        if (description && description.includes("DIMENSIONS:")) {
          const dimMatch = description.match(/DIMENSIONS:({.*?})$/);
          if (dimMatch) {
            try {
              dimensions = JSON.parse(dimMatch[1]);
  ...
  private toAirtableItemFields(itemData: any): any {
    ...
    // Customer field is a linked record field in Airtable, expects a single record ID string
    ...
    // Re-enable Estimated Value - it's critical for insurance calculations
    ...
    // Add dimension and weight fields (store in Description field as JSON for now)
    if (itemData.length !== undefined || itemData.width !== undefined || 
        itemData.height !== undefined || itemData.weight !== undefined) {
      const dimensions = {
      ...
      // Store dimensions in Description field along with any existing description
      const existingDesc = itemData.description || "";
      airtableData["Description"] = existingDesc + 
        (existingDesc ? "\n" : "") + 
...
// Helper function for accessing the base directly
