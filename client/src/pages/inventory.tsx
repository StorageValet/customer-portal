...
export default function Inventory() {
  ...
  const handleSchedulePickup = () => {
    ...
    navigate(`/schedule-pickup?items=${selectedItems.join(",")}`);
  };
  ...
  const handleScheduleDelivery = () => {
    ...
    // Navigate to scheduling page with selected items
    navigate(`/request-delivery?items=${selectedItems.join(",")}`);
  };
            ...
            {isLoading ? (
              ...
              <div className="text-center py-8">
                ...
                {!searchTerm && !categoryFilter && !statusFilter && (
                  <Button
                    onClick={handleAddItem}
                    className="mt-4 bg-turquoise text-white hover:bg-turquoise/90"
                  >
                    Add Your First Item
                  </Button>
                )}
